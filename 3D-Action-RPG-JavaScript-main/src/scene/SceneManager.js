...
import { createCharacterSelectScene } from './scenes/characterSelect.js';
...
class SceneManager {
  constructor(canvasId, engine) {
    ...
    this.engine.enableOfflineSupport = false; // Disable offline cache for testing
    this.engine.doNotHandleContextLost = true; // Better performance
    
    this.guiTextures = new Map();
    this.scenes = [];
    this.activeScene = null;
    this.sceneCreators = {
      characterSelect: createCharacterSelectScene,
      night: createNight,
  ...
  async start() {
    ...
    // Default to character select screen if no scene specified
    const defaultScene = this.sceneCreators[sceneParam] || this.sceneCreators.characterSelect;
