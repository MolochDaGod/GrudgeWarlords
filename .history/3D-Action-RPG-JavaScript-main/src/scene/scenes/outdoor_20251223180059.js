...
export async function createOutdoor(engine) {
    ...
    let sword = addSword(scene, models["Sword2"], skeleton);
    createTrail(scene, engine, sword, 0.2, 40, new BABYLON.Vector3(0, 0, 0.32));

    const slime1 = models["Slime1"];
    setupEnemies(scene, character, terrain, 3, slime1);  // Reduced from 7 to 3 for better performance

    VFX['fireBall'] = addFireball(scene, engine);
    
    // Performance optimizations for testing
    scene.autoClear = false; // Don't clear color buffer
    scene.autoClearDepthAndStencil = false; // Don't clear depth buffer
    scene.blockMaterialDirtyMechanism = true; // Freeze material updates
...
function setupShadows(light, shadowCaster) {
    // Optimized shadow settings for testing
    const shadowGenerator = new BABYLON.ShadowGenerator(512, light); // Reduced from 1024
    ...
    // Performance: limit shadow updates
    shadowGenerator.getShadowMap().refreshRate = 2; // Update every 2 frames
}
...
function setupPostProcessing(scene, camera) {
    // Optimized for testing - reduced quality for better performance
    const pipeline = new BABYLON.DefaultRenderingPipeline(
        "default", // The name of the pipeline
        false,     // HDR disabled for performance
        scene,    // The scene linked to
        [camera]  // The list of cameras to be attached to
    );

    // Configure effects - optimized settings
    pipeline.samples = 2;  // Reduced from 4 for performance
    pipeline.fxaaEnabled = true;   // Keep FXAA for anti-aliasing
